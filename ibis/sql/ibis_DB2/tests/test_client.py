...
def test_list_tables(con):
    ...
    assert len(con.list_tables(like='STORES')) == 1


def test_compile_verify(alltypes):
    unsupported_expr = alltypes.col_int.approx_median()
    assert not unsupported_expr.verify()

    supported_expr = alltypes.col_int.sum()
...
def test_compile_toplevel():
    ...
    expr = t.foo.sum()
    result = ibis.sql.ibis_DB2.api.compile(expr)
...
def test_list_schemas(con):
    assert 'test_sample' in con.list_schemas()
    assert 'test_schema' in con.list_schemas()


def test_metadata_is_per_table():
    con = ibis.sql.ibis_DB2.api.connect(
        host=IBIS_DB2_HOST,
        user=IBIS_DB2_USER,
    ...
    t = con.table('STORES')  # noqa
    assert 'STORES' in con.meta.tables
...
def test_schema_table():
    ...
    assert isinstance(schema['STORES'], ir.TableExpr)
